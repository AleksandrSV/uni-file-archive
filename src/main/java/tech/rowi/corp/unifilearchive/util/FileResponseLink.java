...
import tech.rowi.corp.unifilearchive.controller.FileController;
...
public class FileResponseLink implements LinkUtils<FileResponse> {
    public FileResponse addOperationWithLink(FileResponse response) {
        Link[] links = new Link[]{
                WebMvcLinkBuilder.linkTo(methodOn(FileController.class).getFileDetail(response.getFilename()))
                        .withRel("file")
                        .withType("GET")
